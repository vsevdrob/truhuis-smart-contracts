...
import {
    TruhuisMarketplaceStorage as Storage
} from "./TruhuisMarketplaceStorage.sol";
import "../../libraries/IdentificationLogic.sol";
import "./stages/AvailableStage.sol";
import "./stages/NegotiationStage.sol";
    ...
    NegotiationStage
{
    ...
    function acceptOffer(
        ...
        uint256 _offerId,
        uint256 _tokenId
    ) external override nonReentrant onlySeller(msg.sender, _tokenId) {
        Listing storage sListing = _sGetListing(_tokenId);

        if (sListing.stage == Stage.available) {
            _acceptOffer__AvailableStage(_offerer, _offerId, sListing);
        }
    }
        ...
        onlySeller(msg.sender, _tokenId)
    {
        ...
        if (sListing.stage == Stage.available) {
            _cancelListing__AvailableStage(_tokenId, sListing);
        }

        if (sListing.stage == Stage.negotiation) {
            _cancelListing__NegotiationStage(_tokenId, sListing);
        }
    }
        ...
        onlySeller(msg.sender, _tokenId)
    {
        ...
        if (sListing.stage == Stage.negotiation) {
            _cancelAcceptedOffer__NegotiationStage(sListing);
        }
    }
        ...
        onlyBuyer(msg.sender, _tokenId)
    {
        ...
        if (sListing.stage == Stage.available) {
            _cancelOffer__AvailableStage(_offerId, sListing);
        }

        if (sListing.stage == Stage.negotiation) {
            _cancelOffer__NegotiationStage(_offerId, sListing);
        }
    }
    ...
    function createOffer(
        ...
        uint256 _price,
        uint256 _tokenId
    ) external onlyBuyer(msg.sender, _tokenId) {
        Listing storage sListing = _sGetListing(_tokenId);
    ...
    function listRealEstate(
        ...
        uint256 _tokenId,
        uint256 _price
    ) external onlySeller(msg.sender, _tokenId) {
    ...
    /// @inheritdoc ITruhuisMarketplace
    function updateMarketplaceServiceFee(uint96 _newServiceFee)
        external
        onlyOwner
    {
        if (_newServiceFee == _sMarketplaceServiceFee) {
            revert MarketplaceServiceFeeUpdateFailed();
        }

        _sMarketplaceServiceFee = _newServiceFee;
        emit MarketplaceServiceFeeUpdated(_newServiceFee);
    }
        ...
        onlySeller(msg.sender, _tokenId)
    {
        ...
        if (sListing.stage == Stage.available) {
            _updateListingPrice__AvailableStage(_tokenId, _newPrice, sListing);
        }
    }
